;(function () {
  ...
  function FontBoosterFixer(options) {
     ...
     * @type Array
     */
    this.styleAttributes = options.styleAttributes || ['font-size', 'line-height'];
  ...
  FontBoosterFixer.prototype.convertStyle = function(style) {
    for(var i = 0; i < this.styleAttributes.length; i++) {
      if(style[this.styleAttributes[i]]) {
        style[this.styleAttributes[i]] = this.convertSize(style[this.styleAttributes[i]]);
      }
    }
  };
  ...
  /**
   * Trigger script by default on load
   *
   */
  if ('addEventListener' in document) {
    document.addEventListener('DOMContentLoaded',function(){
      if(window && !window.disableFontBoosterFixer) {
        (new FontBoosterFixer(window.fontBoosterFixerOptions || {})).parsePage();
      }
    });
  }
